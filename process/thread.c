...
static u64 load_binary(struct process *process,
    ...
    for (i = 0; i < elf->header.e_phnum; ++i) {
        ...
        if (elf->p_headers[i].p_type == PT_LOAD) {
            ...
            /* the size needed to be copied */
            seg_sz = elf->p_headers[i].p_filesz;
            /* memsz include variables in .bss */
            seg_map_sz = elf->p_headers[i].p_memsz + p_vaddr % PAGE_SIZE;
            /* pmo init will roundup size */
            ...
            // copy the data from binary to pmo
            memcpy((void *) phys_to_virt(pmo->start) + p_vaddr % PAGE_SIZE,
                   bin + elf->p_headers[i].p_offset, seg_sz);
