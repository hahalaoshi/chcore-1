...
MU_TEST(test_mapping_block) {
        ...
        vmr_prop_t flags = 3;
        flags |= L2_BLOCK;
        err = map_range_in_pgtbl(root, va, 0x200000, 1<<21, flags);
        err = query_in_pgtbl(root, va, &pa, &entry);
        mu_assert_int_eq(BLOCK_PTP, err);
        mu_check(!entry->l2_block.is_table);
        mu_assert_int_eq(entry->l2_block.pfn, 1);
        mu_check(pa == 0x200000);
        err = unmap_range_in_pgtbl(root, va, 1<<21);
        mu_assert_int_eq(0, err);
        err = map_range_in_pgtbl(root, va, 0x300000, 1<<12, 3);
        mu_assert_int_eq(0, err);
        err = query_in_pgtbl(root, va, &pa, &entry);
        mu_assert_int_eq(0, err);
        mu_assert_int_eq(pa, 0x300000);
...
MU_TEST_SUITE(test_suite)
        {
                ...
                /* test block mapping */
//                MU_RUN_TEST(test_mapping_block);
        }
