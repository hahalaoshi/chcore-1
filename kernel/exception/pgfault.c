...
int handle_trans_fault(struct vmspace *vmspace, vaddr_t fault_addr) {
    ...
    if (vmr == NULL) {
        kdebug("Vmr not found!\n");
        return -ENOMAPPING;
    }
    pmo = vmr->pmo;
    if (pmo == NULL || pmo->type != PMO_ANONYM) {
        kdebug("PMO non anonym!\n");
        return -ENOMAPPING;
    }
    len = PAGE_SIZE;
    pa = (paddr_t)virt_to_phys(kmalloc(len));
    pmo->size = PAGE_SIZE;
    pmo->start = pa;
    map_range_in_pgtbl(vmspace->pgtbl, fault_addr, pa, len, vmr->perm);
    return 0;
}
