...
int rr_sched_enqueue(struct thread *thread) {
    ...
    if (aff == NO_AFF) {
        aff = smp_get_cpu_id();
    } else if (aff < 0 || aff >= PLAT_CPU_NUM) {
        // invalid aff
        return -1;
    }
...
//    target = container_of(rr_ready_queue[smp_get_cpu_id()].next,
//                          struct thread, ready_queue_node);
//    kdebug("sched cpuid:%d ready queue\n", smp_get_cpu_id());
    /* traverse the list to find one */
    for_each_in_list(target, struct thread, ready_queue_node, &rr_ready_queue[smp_get_cpu_id()]) {
//        if (target == NULL) {
//            kdebug("NULL...");
//        } else if (!target->thread_ctx) {
//            kdebug("No context...");
//        } else
//           kdebug("state:%d type:%d cpu: %d", target->thread_ctx->state, target->thread_ctx->type, target->thread_ctx->cpuid);
        if (!target || !target->thread_ctx ||
            target->thread_ctx->type == TYPE_IDLE ||
            target->thread_ctx->type > TYPE_TESTS ||
            target->thread_ctx->state != TS_READY) {
        } else {
            list_del(&target->ready_queue_node);
//            list_del(rr_ready_queue[smp_get_cpu_id()].next);
            target->thread_ctx->state = TS_RUNNING;
            return target;
        }
    }
    target = &idle_threads[smp_get_cpu_id()];
    return target;
}
 ...
 */
int rr_sched(void) {
    // current thread can be null if is called from sys_exit
...
void rr_sched_handle_timer_irq(void) {
    // seems like the test allows it to be 0
    // but the type of budget is unsigned int
