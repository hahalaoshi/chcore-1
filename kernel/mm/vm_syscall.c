...
u64 sys_handle_brk(u64 addr) {
    ...
    if (addr == 0) {
        ...
        if (!pmo) {
            ...
            retval = vmspace->heap_vmr->start;
        }
    } else if (addr - vmr->start >=  vmr->size) {
        pmo = vmr->pmo;
        // update the size of vmspace->heap_vmr and the size of its corresponding pmo
        pmo->size += ROUND_UP(addr - vmr->start - vmr->size + 1, PAGE_SIZE);
        vmr->size += ROUND_UP(addr - vmr->start - vmr->size + 1, PAGE_SIZE);
