...
//    map_range_in_pgtbl(pgtbl, va, pa, len, flags);
    ...
    /* mapping in L2_BLOCK*/
    len += va % ARM64_MMU_L2_BLOCK_SIZE;
    pa -= va % ARM64_MMU_L2_BLOCK_SIZE;
    va -= va % ARM64_MMU_L2_BLOCK_SIZE;
    while (len > 0) {
        ptp_t *pgtbl = (ptp_t *) get_ttbr1();
        pgtbl = (ptp_t *) phys_to_virt(pgtbl);
        pte_t *entry;
        entry = &(pgtbl->ent[GET_L0_INDEX(va)]);
        pgtbl = (ptp_t *) phys_to_virt(entry->table.next_table_addr << PAGE_SHIFT);
        entry = &(pgtbl->ent[GET_L1_INDEX(va)]);
        pgtbl = (ptp_t *) phys_to_virt(entry->table.next_table_addr << PAGE_SHIFT);
        entry = &(pgtbl->ent[GET_L2_INDEX(va)]);
        ...
        entry->l2_block.pfn = pa >> (ARM64_MMU_L2_BLOCK_ORDER + PAGE_SHIFT);
        va += ARM64_MMU_L2_BLOCK_SIZE;
        pa += ARM64_MMU_L2_BLOCK_SIZE;
