...
typedef union {
	...
	struct {
		u64 is_valid        : 1,
		    is_table        : 1,   // should be 0 for a block
	...
	/* 2M block, only difference may be the pfn length*/
	struct {
		u64 is_valid        : 1,
		    is_table        : 1,
		    attr_index      : 3,   // Memory attributes index
...
/* 1 page table page contains one page table*/
typedef struct {
/* index size of a page table 4KB*/
