...
int main(int argc, char *argv[], char *envp[]) {
    ...
    // The following block is almost the same as ipc_reg.c
    {
        ...
        ret = spawn("/ipc_recv_server.bin", &new_process_cap, &new_thread_cap,
    ...
    int times = 0;
    {
        while (1) {
            times++;
            ret = ipc_send(&client_ipc_struct, times * 100);
            if (ret != 0) {
                break;
            }
        }
    ...
    printf("[Sender] send message %lu, send times: %d \n", times * 100, times);
    printf("[Sender] exit\n");
    info_page->exit_flag = 1;
    return 0;
}
